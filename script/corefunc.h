	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("add_label_with_icon|big|`wGet a GrowID``|left|206|") != std::string::npos) {
	    				SendPacket(2, "action|dialog_return\ndialog_name|growid_apply\nlogon|"+ GrowidDepan + to_string(START_DARI) + GrowidBelakang + "\npassword|"+ Password_acc +"\npassword_verify|"+ Password_acc +"\nemail|" + Email_acc + to_string(START_DARI) + "@gmail.com", peer);
              	}
	    				...
	    				appendAcc("GrowID : " + growid + "\nPassword : " + Password_acc + "\nEmail : " + Email_acc + to_string(START_DARI) + "@gmail.com");
	    				if(START_DARI == END_NUMBER){
	    					exit(0);
	    				}
